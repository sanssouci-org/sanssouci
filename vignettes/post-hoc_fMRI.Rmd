...
TP_Simes <- bound(res_Simes, what = "TP")
```

The Simes bound implies that with probability larger than `r 1-alpha`, the number of voxels associated to the task of interest (among all `r nHyp(obj)` voxels) is at least  `r TP`.
...
B <- 1000
res <- fit(obj, alpha = alpha, B = B, refFamily = "Simes")
```
...
K <- 500
res_Beta <- fit(obj, alpha = alpha, B = B, refFamily = "Beta", K = K)
```
...
bounds <- sapply(resList, bound, what = "TP")
...
```{r plot-conf-bounds}
conf_bounds <- lapply(resList, bound, all = TRUE)
cols <- RColorBrewer::brewer.pal(length(conf_bounds), "Dark2")
p <- plotConfCurve(conf_bounds, xmax = xmax, cols = cols)
