...
```{r}
#| label: plot-conf-env
plotConfCurve(confs, xmax = 200, legend.title = "Template")
```
...
```{r}
#| label: sim-one-sample
obj <- SansSouciSim(m, rho, n, pi0, SNR = SNR)
obj
```

```{r}
#| label: cal-one-sample
cal <- fit(obj, alpha = alpha, B = B, family = "Simes")
cal
```
...
```{r}
#| label: other-conf-env-one-sample
cal0 <- fit(obj, alpha = alpha, B = 0, family = "Simes")
oracle <- fit(obj, alpha = alpha, family = "Oracle")
...
```{r}
#| label: plot-conf-env-one-sample
plotConfCurve(confs, legend.title = "Template")
```
