---
title: "Step-down Joint Error Rate control"
...
```{r JER}
alpha <- 0.1
res <- sansSouci:::calibrateJER0(p0, refFamily = "kFWER", alpha=alpha, p.values = p)
thr <- res$thr
...
o <- order(p)
po <- p[o]
bounds <- apply(thrMat, 2, function(thr) sansSouci:::curveMaxFP(po, thr))
```
...
p0.Oracle <- p0[-H1, ]
p.Oracle <- p[-H1]
res.Oracle <- sansSouci:::calibrateJER0(p0.Oracle, refFamily = "kFWER", alpha = alpha, p.values = p.Oracle)
thrO <- c(res.Oracle$thr, rep(1, length(H1)))
po.Oracle <- p.Oracle[order(p.Oracle)]
VbarO <- sansSouci:::curveMaxFP(po, thrO)
```
