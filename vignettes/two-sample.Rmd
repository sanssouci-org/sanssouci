...
cal_beta <- calibrateJER(X = dat, B = 100, alpha = alpha, refFamily = "Beta", K = 50)
ce_Beta <- cal_beta$conf_env
```
...
ce <- confidenceEnvelope(cal_beta$stat, refFamily = "Beta", param = cal_beta$lambda, what = "TP")
```
...
ggplot(subset(conf_env, x <= xmax), 
            aes(x = x, y = bound, color = procedure, linetype = procedure)) +
  geom_line() +
  facet_wrap(~ stat, scales = "free_y") + 
  labs(x = "# top genes called significant", y = "Post hoc confidence bounds") +
  theme_bw() + 
