...
bh_plot + 
  ggsave(filename = "BH-plot.png", path = "~/Downloads/",
                  width = 12, height = 8)
```
...
bh_plot + 
  ...
  ggsave(filename = "BH-plot_zoom.png", path = "~/Downloads/",
         width = 12, height = 8)
```
...
The method of @GS2011 makes it possible to calculate a confidence envelope on the number of true positives among the most significant genes. In the framework of @blanchard:posthoc this bound is obtained from simes86improved inequality, by interpolation:

```{r plot-conf-envelope}
Kmax <- 200
conf_env <- confidenceEnvelope(stat = dex$statistic, family = 'Simes(0.2)', what = c("TP", "FDP")) %>% 
  filter(x <= Kmax)
p <- ggplot(conf_env, aes(x = x, y = bound)) +
  geom_line() +
...
```{r TP_BH, echo=FALSE}
TP_BH <- subset(conf_env, x==nBH & stat == "TP")$bound
```
...
However, as discussed in @blanchard:posthoc, this bound has two major limitations, being a consequence of Simes' inequality:
...
To bypass these limitations, @blanchard:posthoc have proposed a randomization-based procedure known as $\lambda$-calibration, which yields tighter bounds that are adapted to the dependency observed in the data set at hand. In the case of two-sample tests, this calibration is achieved by permutation of class labels:

```{r calibration}
cal <- calibrateJER(X = dat, B = 100, alpha = alpha, refFamily = "Simes")
#cal_beta <- calibrateJER(X = dat, B = 100, alpha = alpha, refFamily = "Beta")
```


```{r calculate-bound}
famz <- list("Simes" = toFamily("Simes", alpha), 
                 "Simes + calibration" = cal$thr, 
                 "Beta" = toFamily("Beta", alpha))
#                 "Beta + calibration" = cal_beta$thr)
conf_env <- confidenceEnvelope(stat = cal$stat, family = famz, what = c("TP", "FDP"))
```


```{r, conf-env-plot}
env <- ggplot(subset(conf_env, x <= 200), aes(x = x, y = bound, color = family, group = family)) + 
  geom_line() +
  facet_grid(stat ~ ., scales = "free_y") +
  labs(x = "#top genes called significant", y = "")
