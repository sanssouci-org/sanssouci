...
vignette: |
  %\VignetteIndexEntry{Post hoc inference for differential gene expression studies} %\VignetteEncoding{UTF-8} %\VignetteEngine{knitr::rmarkdown}
---
...
Set RNG seed for numerical reproducibility of the results:
```{r set-seed}
set.seed(20200924)
```
...
```{r read-data}
filename <- "~/Downloads/chiaretti_leukemia.rds"
...
cal <- calibrateJER(X = dat, categ, B = 1000, alpha = alpha, refFamily = "Simes")
```
...
K <- 50
cal_beta <- calibrateJER(X = dat, categ, B = 1000, alpha = alpha, refFamily = "Beta", K = K)
```
...
bounds <- c("k0-Bonferroni" = FP_k0, 
         "Simes" = FP_Simes,
         "Simes + calibration" = FP_SimesCal,
         "Beta + calibration" = FP_BetaCal)
tab <- data.frame("Method" = names(bounds), 
                  "bound on True Positives >=" = nBH - bounds, 
                  "bound on False Discovery Proportion <=" = ceiling(bounds/nBH*100)/100,
...
p + geom_vline(xintercept = nBH, color = "gray", linetype = "dotted") 
```
...
all_bounds <- function(pvals, thrs) {
  c(n = length(pvals), sapply(thrs, function(thr) minTP(pvals, thr)))
}
...
tab <- cbind(c("BH-adjusted p.value < q and fold change > r",
               ...
               "BH-adjusted p.value < q and |fold change| > r"), 
             tab)
knitr::kable(tab, caption = "Post hoc bounds on the number of true positives in user-defined gene selections")
```
