---
title: "Bootstrap-based post hoc inference for differential gene expression studies in linear model context with microarray data"
...
vignette: |
  %\VignetteIndexEntry{Bootstrap-based post hoc inference for differential gene expression studies in linear model context with microarray data}
...
This vignette illustrates the relevance of bootstrap-based post hoc bounds on false positives for the differential analysis of gene expression measurements (microarray data). We illustrate how the post hoc methods introduced by @blanchard20post-hoc and extended to linear model by @davenport2022fdp may be used to 
...
The methods described in this vignette have been extended by @davenport2022fdp for complex experimental design witch need linear models. In this vignette we reproduce results in @davenport2022fdp.
...
The data consists of $D = 12,531$ gene expression. 
For each gene, we fit a linear model including the pulmonary obstruction ratio (`fev1_fvc_utah`), along with the covariates `age_enroll`, `gender`, `bmi` (body mass index), `parentalcopd` (parental history of COPD), `smokcignow` (smoking status), and `ats_packyears` (pack-years). The design matrix $X$ includes an intercept term.
...
For each gene, we fit a model to estimate parameters $\beta$. Then we test for each gene $j$ and for the contrast $c$, if $\mathcal{H}_0: c\beta = 0$. In our case, we test if the parameter of the studied variable `fev1_fvc_utah` is equal to 0. 
We want to obtain post hoc guarantees on a set of genes which are in interaction with `fev1_fvc_utah`. We first build a `SansSouci` class object given the gene expression matrix $Y$, the design matrix $X$ and the contrast matrix $C$. The we fit the model and calibrate the post hoc bound using the method `fit()` with $B = 1000$ bootstrap and the target risk such that $\alpha = 0.1$.


```{r}
alpha = 0.1
obj <- SansSouci(Y = Y, X = X, Contrast = C)
res <- fit(obj, alpha = alpha, B = 1000)
```
...
This plot can be interpreted as follows: among the 2000 most significant genes, the number of truly differentially expressed genes is at least 1060 (right panel). Equivalently, the FDP among these 2000 genes is at most 0.47 (left panel).

The dashed gray curve is obtained by the parametric Simes method introduced by Goeman and Solari (2011). The comparison between the two curves illustrates the gain in power obtained by using permutations methods to adapt to the dependence between genes. In this example, the parametric Simes method gives the following guarantees: among the 2000 most significant genes, the number of truly differentially expressed genes is at least 492 (right panel). Equivalently, the FDP among these 2000 genes is at most 0.754 (left panel).
...
The above example also illustrates the increase in power obtained by calibration, since the parametric Simes method yields a subset of "only" `n_DEG[["Simes (parametric)"]]` genes called differentially expressed (with identical statistical guarantees).
...
If several contrasts will be studied, to obtain post hoc guarantees, we have to consider all the tests (for all genes and all tested contrasts) simultaneously to calibrate the post hoc method.
The contrast matrix $C$ contains all the contrast, one by row. For example, we want to test the lung function variable `fev1_fvc_utah` and the `age_enroll` variable. Then the contrasts matrix is : 
...
res <- fit(object = object, alpha = alpha)
```
...
> Which genes are differentially expressed for lung function with high probability?

To do so, we *define* differentially expressed genes as the largest set of genes for which the FDP bound is less than a user-given value, for example $q = 0.1$. This corresponds to drawing a horizontal line in the confidance curves on the FDP:
...
```{r}
volcanoPlot(x = res, contrast = "age_enroll", p = 1e-3, r = 0.015)
```
