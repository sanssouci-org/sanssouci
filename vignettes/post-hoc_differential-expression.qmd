---
title: "Permutation-based post hoc inference for two-group differential gene expression studies"
...
vignette: |
  %\VignetteIndexEntry{Permutation-based post hoc inference for two-group differential gene expression studies}
...
The methods described in this vignette are described in detail in the book chapter @BNR:chap and in the paper @blanchard20post-hoc. A shiny application for volcano plots is also available at <https://shiny-iidea-sanssouci.apps.math.cnrs.fr/>.
...
bh_plot <- ggplot(dexo, aes(x = gene_order, y = p.value)) +
  ...
  geom_abline(slope = 1/m, intercept = 0, linetype = 2, linewidth = 1) +
  geom_abline(slope = q/m, color = my_col, linewidth = 2) +
  ...
  geom_abline(slope = 0, intercept = q, linetype = "dotted", color = my_col, linewidth = 2) +
  theme_bw() +
...
p + geom_vline(xintercept = nBH, color = "gray", linetype = "dotted", linewidth = 1.5) +
...
```{r}
#| label: tbl-compare-volcano-bounds
#| tbl-cap: "Post hoc bounds on true positives in user-defined gene selections"
fc <- foldChanges(res)
...
#| eval: false
quarto::quarto_render("post-hoc_differential-expression.qmd", output_format = "pdf")
# To keep intermediate files, add option 'clean = FALSE' (rmarkdown-based rendering)
quarto::quarto_render("post-hoc_differential-expression.qmd", output_format = "html")
```
