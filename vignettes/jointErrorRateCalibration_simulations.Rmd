...
We use the function `SansSouciSamples` to generate Gaussian equi-correlated samples. We then 'fit' the resulting SansSouci object
...
```{r simulation}
obj <- SansSouciSamples(m, rho, n, pi0, SNR = SNR, prob = 0.5)
obj
```
...
B <- 500
alpha <- 0.2
cal <- fit(obj, alpha = alpha, B = B, refFamily = "Simes")
```

The output of the calibration is the following `SansSouci` object:
...
confs <- list(Simes = bound(cal0, all = TRUE),
              "Simes+calibration" = bound(cal, all = TRUE),
...
```{r, sim-one-sampla}
obj <- SansSouciSamples(m, rho, n, pi0, SNR = SNR)
obj
```
