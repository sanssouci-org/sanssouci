...
```{r}
#| label: post-hoc-calibration-beta
#| cache: true
K <- 500
...
```{r}
#| label: posthoc-bounds-atlas-table
...
#| eval: false
quarto::quarto_render("post-hoc_fMRI.qmd", output_format = "pdf")
# To keep intermediate files, add option 'clean = FALSE' (rmarkdown-based rendering)
quarto::quarto_render("post-hoc_fMRI.qmd", output_format = "html")
```
