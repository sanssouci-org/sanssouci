...
<span style="color:red"> Quelle intro faire pour la  vignette ? Est ce qu'on fait le lien avec la vignette précédente en disant que c'est une adaptation aux données RNAseq ? </span>
...
require("curatedTCGAData") || BiocManager::install("curatedTCGAData")
require("TCGAutils") || BiocManager::install("TCGAutils")
```
...
Post hoc bounds are calculated on any independent selection. Until now, the volcano plot used to select the genes used the same statistical test as the one used for the post hoc bounds.  Nevertheless, this two tests could be differents because they are independant of each other. It is possible to print the volcano plot with a first test and estimate post hoc bound with an other test. The next plot shows a volcanoplot with p-values and log-Fold changes from limma-voom methods (@) and post hoc bounds estimates with the wilcoxon's test: 
...
tmp <- eBayes(tmp)
TT <- topTable(tmp, sort.by = "none", n = Inf)
p.value <- as.numeric(TT$P.Value)
logFC <- as.numeric(TT$logFC)

volcanoPlot(res_Simes, fold_changes = logFC, p_values = p.value, q = q, r = r) 
```
