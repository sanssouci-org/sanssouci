...
This vignette addresses the problem of testing contrasts in linear models arising from the differential analysis of gene expression measurements (microarray data). We illustrate how the post hoc methods introduced by @blanchard20post-hoc and extended to linear models by @davenport2025fdp may be used to
...
In this vignette we reproduce some of the results obtained in Section 5.2 of @davenport2025fdp. These bootstrap-based methods are *not yet implemented* in the shiny application for volcano plots available at <https://shiny-iidea-sanssouci.apps.math.cnrs.fr/>.
...
p <- plotConfCurve(FDP, xmax = 2.5*n_DEG, col = cols, legend.title = "Template") +
  geom_hline(yintercept = q, linetype = "dashed", linewidth = linewidth) +
  geom_vline(xintercept = n_DEG, linetype = "dotted", col = cols, linewidth = linewidth) +
...
The contrast matrix $C$ contains all contrasts, one by row. For example, let us assume that we we want to test the lung function variable `fev1_fvc_utah` and the `age_enroll` variable. Then the contrast matrix is then:
...
We build another `SansSouci` object with this contrast matrix, fit the model and calibrate the post hoc bounds.
...
By default, the confidence curves associated to each contrast are displayed on the same plot, allowing for direct comparison of the signal across multiple contrasts. In the example below, we can clearly observe that the signal associated with the `fev1_fvc_utah` variable is stronger than that of `age_enroll`.
...
p <- plot(res, xmax = 2.5*n_DEG, contrast_name = c("age_enroll", "fev1_fvc_utah"), what = c("FDP")) +
  geom_hline(yintercept = q, linetype = "dashed", linewidth = linewidth) +
  geom_vline(xintercept = n_DEG, linetype = "dotted",  linewidth = linewidth) +
  geom_line(linewidth = linewidth)
p
```

It is also possible to specify the contrast of interest. To do so for the lung function (corresponding to the contrast `fev1_fvc_utah`), we *define* differentially expressed genes as the largest set of genes for which the FDP bound is less than a user-given value, for example $q = 0.1$. This corresponds to drawing a horizontal line in the confidence curves on the FDP:

```{r}
#| label: post-hoc-FDP-control-contrast-fev
q <- 0.1 # FDP budget (user-defined)
...
The same analysis can be made for the contrast on `age_enroll`.

```{r}
#| label: post-hoc-FDP-control-contrast-age
...
Due to the weaker signal in this case, we had to increase the target FDP level $q$ in order to obtain a relatively large set of genes selected.
...
```{r}
#| label: volcano-contrast-fev
volcanoPlot(x = res, contrast_name = "fev1_fvc_utah", p = 1e-3, r = 0.5)
```

Compared to the volcano plot obtained in the preceding section, the selected genes are the same because the $p$-values are identical. However, the confidence bounds are less tight. This is explained by the fact that here two contrasts are tested instead of one: the total number of hypotheses tested is twice as large as previously, resulting in a more severe multiple testing correction.

```{r}
#| label: volcano-contrast-age
