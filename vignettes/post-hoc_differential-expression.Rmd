...
res_Simes <- fit(obj, B = 0, refFamily = "Simes", alpha = alpha) ## B=0 => no calibration!
FP_Simes <- bound(res_Simes, S_BH, what = "FP")
```
...
B <- 100
res <- fit(obj, B = B, alpha = alpha, refFamily = "Simes")
```
...
K <- 50
res_Beta <- fit(res, B = B, alpha = alpha, refFamily = "Beta", K = K)
```


As expected from the theory, the post hoc bounds obtained after calibration by these methods is much tighter than the $k_0$-Bonferroni and Simes bounds:
...
bounds <- sapply(resList, bound, S_BH)
...
The confidence envelopes obtained by calibration of the Simes and Beta families can be compared graphically to the (parametric) Simes envelope of @GS2011:

```{r, conf-env-plot}
conf_envs <- lapply(resList, bound, envelope = TRUE)
...
ylim <- c(0, 6)
volcano_plot(res_Simes, q = q, r = r, ylim = ylim)
```
...
volcano_plot(res, q = q, r = r, ylim = ylim)
```

and for the Beta reference family.

```{r volcano-plot-Beta-calibrated}
volcano_plot(res_Beta, q = q, r = r, ylim = ylim)
```
...
all_bounds <- function(S, resList) {
  c(length(S), sapply(resList, bound, S, "TP"))
}
