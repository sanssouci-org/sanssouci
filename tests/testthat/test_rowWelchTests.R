...
test_that("rowWelchTests gives identical results to welch.test", {
    ...
    for (alt in c("two.sided", "greater", "less")) {
        ...
        wt <- apply(mat, 1, FUN = function(x) {
            tt <- t.test(x[cls == 0], x[cls == 1], alternative = alt)  ## test stat positive if "1 > 0"
        ...
        # Formula interface (not using the tidyverse)
        wt <- apply(mat, 1, FUN = function(x) {
            df <- cbind(x, cls)
            tt <- t.test(x~cls, data = df, alternative = alt)  ## test stat positive if "1 > 0"
            c(tt$p.value, tt$statistic, tt$parameter)
        })
        dwt <- as.data.frame(t(wt))
        names(dwt) <- c("p.value", "statistic", "parameter")
