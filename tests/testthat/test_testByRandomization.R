...
test_that("Consistency of randomization p-values with different alternatives", {
    for (alt in c("two.sided", "less", "greater")) {
        ...
        two.sidedInv <- testByRandomization(X = mat, B = 100, alternative = "two.sided")
        greaterInv <- testByRandomization(X = mat, B = 100, alternative = "greater")
        lessInv <- testByRandomization(X = mat, B = 100, alternative = "less")
