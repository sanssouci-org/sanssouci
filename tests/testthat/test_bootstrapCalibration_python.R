...
test_that("Consistency of lm_test with pyperm package", {
  ...
  for (D in DD){
    for (p in PP){
      ...
      expect_equal(t(tstats_py[[1]]$field), tstats_R$stat_test, tolerance = 1e-14)
      expect_equal(as.numeric(t(tstats_py[[2]])), 
                   as.numeric(tstats_R$epsilon_est), tolerance = 1e-14)
      
    }
  }
})

test_that('Consistency of `bootstrap_permutation`', {
  skip_if_not(py_module_available("pyperm"), "Module pyperm not available")
  ...
  for (cc in seq_configs) {
    ...
    # Sigma = matrix(1, nrow = D, ncol = D) #too much correlation but works for toelrance at 1e-10
    ...
    res_py <- pyperm$boot_contrasts(lat_data = t(Y), design = X, contrast_matrix = C,
